...
`Owning(p)` reads as "the raw pointer `p` is currently the sole carrier of the ownership of `*p`": no live owner object aliases the pointee (its previous owner was dissolved, e.g. via `into_raw`/`leak`), so the callee may soundly (re)claim ownership. Violating it — reconstructing an owner while another owner is still alive — leads to double free.
